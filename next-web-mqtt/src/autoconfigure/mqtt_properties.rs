...
pub struct MQTTClientProperties {
    ...
    /// Capacity=(message generation rate - message processing rate) x buffer time
    ///
    /// `cap指定了有界异步通道的容量
    /// 容量 = (消息生成率-消息处理率) x 缓冲时间
    cap: Option<usize>,

    /// Maximum number of retries (optional)
    /// When the maximum number of retries is reached, the application will exit
    ///
    /// 最大重试次数(可选)
    /// 当达到最大重试次数时，应用程序将退出
...
impl MQTTClientProperties {
    ...
    /// Returns keep alive interval in seconds
    ///
    /// 返回设置的保活间隔时间(秒)
    ...
    /// Returns capacity of the bounded async channel.
    ///
    /// 返回设置的有界异步通道的容量
    ...
    ///
    /// 返回设置的最大重试次数
    pub fn max_retries(&self) -> Option<u16> {
        self.max_retries
    }
    ...
    pub fn set_max_retries(&mut self, max_retries: u16) {
