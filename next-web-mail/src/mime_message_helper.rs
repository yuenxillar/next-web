...
impl MimeMessageHelper {
    ...
    pub fn set_text(&mut self, text: impl AsRef<str>, html: bool) -> Result<(), MessagingError> {
    ...
    pub fn add_inline_named(
        ...
        if name.is_empty() {
            return Err(MessagingError::EmptyValue("name"));
        }

        let mime_type = normalize_content_type(content_type.as_ref(), Some(name));
        self.message
            .add_inline_resource(content_id, Some(name.to_string()), content, &mime_type);
        Ok(())
    }
    ...
    pub fn set_priority(&mut self, priority: u8) -> Result<(), MessagingError> {
        if !(1..=5).contains(&priority) {
            return Err(MessagingError::EmptyValue(
                "priority must be between 1 and 5",
            ));
        }
        self.message.set_header("X-Priority", priority.to_string());
        Ok(())
    }
...
mod tests {
    ...
    fn write_temp_file(extension: &str, bytes: &[u8]) -> PathBuf {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let path = std::env::temp_dir().join(format!("next-web-mail-{unique}.{extension}"));
    ...
    #[test]
    fn supports_attachment_from_file() {
        let path = write_temp_file("txt", b"attachment-body");

        let mut helper = MimeMessageHelper::new(true, Some("UTF-8".to_string()));
        helper
            .add_attachment_file("report.txt", &path)
            .expect("attachment file should be accepted");
        ...
        assert_eq!(attachment.content, b"attachment-body");

        let _ = fs::remove_file(path);
    }

    #[test]
    fn supports_inline_resource_from_file() {
        let path = write_temp_file("png", &[0x89, b'P', b'N', b'G']);

        let mut helper = MimeMessageHelper::new(true, Some("UTF-8".to_string()));
        helper
            .add_inline_file("logo", &path)
            .expect("inline file should be accepted");

        let message = helper.get_message();
        assert_eq!(message.inline_resources().len(), 1);

        let inline = &message.inline_resources()[0];
        ...
        assert_eq!(inline.content, vec![0x89, b'P', b'N', b'G']);

        let _ = fs::remove_file(path);
    }

    #[test]
    fn supports_named_inline_resource_from_bytes() {
        let mut helper = MimeMessageHelper::new(true, Some("UTF-8".to_string()));
        helper
        ...
        assert_eq!(inline.mime_type, "image/png");
        assert_eq!(inline.content, vec![1, 2, 3]);
    }
}
