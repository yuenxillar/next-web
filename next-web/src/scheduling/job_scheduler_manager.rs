...
use next_web_core::{error::BoxError, traits::singleton::Singleton, util::time::TimeUnit};
...
use super::{
    JobExecutionContext, PersistedScheduledJob, ScheduledJobRegistry, ScheduledJobHandler,
    ScheduledJobReader, ScheduledJobRepository, ScheduledJobStore, InMemoryScheduledJobRepository,
    ScheduleType, WithArgs,
};
...
impl JobSchedulerManager {
    ...
    /// Registers the handler a task is run by.
    ///
    /// The handler is reachable by the key of the task from then on, which is
    /// what lets a schedule that is restored from the repository be run without
    /// the code that declared the task being involved.
    ///
    /// # Arguments
    ///
    ...
    /// # Errors
    ///
    /// Returns an error when a handler is already registered under the key of
    /// the task.
    pub fn register_handler(&self, handler: Arc<dyn ScheduledJobHandler>) -> Result<(), BoxError> {
        self.registry.register(handler)
    }

    /// Writes the schedule an application declared to the repository, without
    /// scheduling it.
    ///
    /// The schedule of a task is written once and is restored from the
    /// repository afterwards, which is what keeps a task that an operator
    /// disabled, or the time it last ran at, from being overwritten while the
    /// application starts. A task whose schedule changed in the code of the
    /// application is updated.
    ///
    /// # Arguments
    ///
    /// * `task_key` - The key the task is registered and persisted under.
    /// * `schedule` - The schedule of the task.
    /// * `enabled` - Whether the task runs.
    /// * `payload` - The payload the handler of the task receives.
    pub async fn declare_schedule(
        ...
        let stored = self.reader.find(task_key).await?;
        ...
        match stored {
            ...
            None => {
                let mut job = PersistedScheduledJob::new(task_key, task_key, schedule);
    ...
    pub async fn restore_all(&self, context: JobExecutionContext) -> Result<(), BoxError> {
        for job in self.reader.read_enabled().await? {
        ...
        T: FnMut(Uuid, JobScheduler) -> Pin<Box<dyn Future<Output = ()> + Send>> + Send + Sync,
    {
        match schedule {
            ...
            ScheduleType::FixedRate(args) => Job::new_repeated_async(
                Self::repeat_interval(Self::duration_from_fixed_rate(&args)?),
                run,
            ),
            ScheduleType::OneShot(args) => Job::new_one_shot_async(
                Self::schedule_delay(Self::duration_from_initial_delay(&args)?),
                run,
            ),
        }
    }
        ...
        T: FnMut(Uuid, JobScheduler) + Send + Sync,
    {
        match schedule {
            ...
            ScheduleType::FixedRate(args) => Job::new_repeated(
                Self::repeat_interval(Self::duration_from_fixed_rate(&args)?),
                run,
            ),
            ScheduleType::OneShot(args) => Job::new_one_shot(
                Self::schedule_delay(Self::duration_from_initial_delay(&args)?),
                run,
            ),
        }
    ...
    /// Returns the interval a task that repeats runs in.
    ///
    /// # Arguments
    ///
    /// * `interval` - The interval the task declared.
    fn repeat_interval(interval: std::time::Duration) -> std::time::Duration {
        Self::round_up_to_a_second(interval, "fixed rate")
    }

    /// Returns the delay before the first run of a task.
    ///
    /// # Arguments
    ///
    /// * `delay` - The delay the task declared.
    fn schedule_delay(delay: std::time::Duration) -> std::time::Duration {
        Self::round_up_to_a_second(delay, "initial delay")
    }

    /// Rounds a duration up to the whole second the scheduler runs at.
    ///
    /// The scheduler of the framework evaluates a task once per second, and it
    /// truncates a duration to whole seconds itself: a task that declares half
    /// a second would be given no time at all, which is a repeated task that
    /// never repeats again and a task that runs immediately instead of after
    /// its delay. The duration is therefore rounded up, and the rounding is
    /// reported, so that a task runs later than declared instead of never.
    ///
    /// # Arguments
    ///
    /// * `duration` - The duration the task declared.
    /// * `name` - The name of the duration, for the report.
    fn round_up_to_a_second(
        duration: std::time::Duration,
        name: &str,
    ) -> std::time::Duration {
        let seconds = duration.as_secs();

        if seconds > 0 && duration == std::time::Duration::from_secs(seconds) {
            return duration;
        }

        let rounded = std::time::Duration::from_secs(seconds.saturating_add(1));

        warn!(
            declared = ?duration,
            scheduled = ?rounded,
            "{name} of a scheduled task is rounded up, the scheduler runs a task once per second"
        );
...
mod tests {
    ...
    use super::{
        BoxedJob, InMemoryScheduledJobRepository, JobExecutionContext, JobSchedulerManager,
        PersistedScheduledJob, ScheduledJobHandler, ScheduledJobRegistry, ScheduledJobReader,
        ScheduledJobStore, ScheduleType, WithArgs,
    };
    ...
    impl ScheduledJobHandler for CountingHandler {
        ...
        async fn execute(
            &self,
        _context: super::JobExecutionContext,
