...
use crate::ws::server::support::ws_session::WebSocketSession;
...
///
pub type WSResult<T> = std::result::Result<T, Box<dyn Error + Send + Sync>>;
    ...
    Self: Send + Sync,
{
    ...
    ///
    async fn on_message(&self, session: &WebSocketSession, message: Message) -> WSResult<()>;
    ...
    ///
    async fn on_close(&self, session: &WebSocketSession, close: Option<CloseFrame>) -> WSResult<()>;
}
