...
fn impl_item_fn(
    ...
    //     ::next_web::configurer::http_method_handler_configurer::HttpMethodHandlerConfigurer::default()
...
fn generate_block(
    ...
    let modify_body = match produce {
        Some(produces) => quote! {
            resp.headers_mut()
            .insert(
                ::next_web::http::header::CONTENT_TYPE,
                ::next_web::http::HeaderValue::from_static(#produces))
            .unwrap();
        },
        None => quote! {},
    };
            ...
            if !consumes.value().trim().is_empty() {
                ...
                quote! {
                    if !__verify_content_type.to_string().contains(#consumes) {
                        return (::next_web::http::StatusCode::UNSUPPORTED_MEDIA_TYPE, "Unsupported Media Type").into_response();
                    }
                }
            } else {
                quote! {}
    ...
    let unified_return_type = match syn::parse2::<syn::ReturnType>(
        quote! { -> impl ::next_web::response::IntoResponse},
    ) {
