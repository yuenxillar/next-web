...
impl CronUtil {
    ...
    pub fn parse(expression: &str) -> Result<CronExpression, CronError> {
        ...
        if parts.len() != 5 {
            return Err(CronError::InvalidExpression(format!(
                "Expected to have 5 fields, actually there are {}",
                parts.len()
            )));
        }
    ...
    fn parse_field(field: &str, min: u8, max: u8) -> Result<Vec<u8>, CronError> {
        ...
        for part in field.split(',') {
            if part.contains('/') {
                ...
                if segments.len() != 2 {
                    return Err(CronError::InvalidField(format!(
    ...
    fn parse_range(range: &str, min: u8, max: u8) -> Result<Vec<u8>, CronError> {
        ...
        if start > end {
            return Err(CronError::InvalidRange(format!(
                "Range start value ({}) greater than end value ({})",
                start, end
            )));
        }
    ...
    pub fn next_execution_from(
        ...
        while candidate < limit {
            ...
            if !cron.day_of_month.contains(&dom) {
                ...
                candidate = candidate.with_hour(0).unwrap().with_minute(0).unwrap();
                continue;
            }
        ...
        Err(CronError::InvalidExpression(
            "Cannot find next execution time, expression may be invalid or never executes"
                .to_string(),
        ))
    }
        ...
        if cron.day_of_week.len() < 7 {
            if cron.day_of_week.len() == 1 {
                desc.push_str(&format!(
                    "Every {} ",
                    Self::day_of_week_name_en(cron.day_of_week[0])
                ));
            } else {
    ...
    pub fn generate_cron(month: i32, week_day: i32, day: i32, hour: i32, minute: i32) -> String {
        let minute_str = if minute < 0 || minute > 59 {
            "*".to_string()
        } else {
            minute.to_string()
        };
        let hour_str = if hour < 0 || hour > 23 {
            "*".to_string()
        } else {
            hour.to_string()
        };
        let day_str = if day < 1 || day > 31 {
            "*".to_string()
        } else {
            day.to_string()
        };
        ...
        format!(
            "{} {} {} {} {}",
            minute_str, hour_str, day_str, month_str, week_day_str
        )
    }
}
