...
pub struct HeadStyle {
    /// Data format index (built-in: see Apache POI BuiltinFormats)
    /// -1 = not set (default)
    data_format: i16,
    ...
    /// Lock/protect the cell
    locked: Option<bool>,

    /// Force Excel to treat value as text (like prefixing with ')
    ...
    /// Text rotation in degrees (-1 = not set)
    /// HSSF: -90..90, XSSF: 0..180 (automatically converted)
    rotation: i16,

    /// Indent level in spaces (-1 = not set)
    indent: i16,
    ...
    // ── Border Colors (IndexedColors index, -1 = not set) ─────
    left_border_color: u16,
    right_border_color: u16,
    top_border_color: u16,
    bottom_border_color: u16,

    // ── Fill ────────────────────────────────────────
    fill_pattern_type: Option<FillPatternType>,
    fill_background_color: u16,
    fill_foreground_color: u16,

    /// Shrink text to fit cell width
    shrink_to_fit: Option<bool>,
}

// 为 HeadStyle 结构体生成的 Getter/Setter 方法
...
impl HeadStyle {
    ...
    pub fn get_left_border_color(&self) -> u16 {
        self.left_border_color
    }

    pub fn get_right_border_color(&self) -> u16 {
        self.right_border_color
    }

    pub fn get_top_border_color(&self) -> u16 {
        self.top_border_color
    }

    pub fn get_bottom_border_color(&self) -> u16 {
    ...
    pub fn get_fill_background_color(&self) -> u16 {
        self.fill_background_color
    }

    pub fn get_fill_foreground_color(&self) -> u16 {
...
impl Default for HeadStyle {
    fn default() -> Self {
        Self {
            ...
            border_bottom: None,
            left_border_color: 0,
            right_border_color: 0,
            top_border_color: 0,
            bottom_border_color: 0,
            fill_pattern_type: None,
            fill_background_color: 0,
            fill_foreground_color: 0,
            shrink_to_fit: None,
        }
    }
}
