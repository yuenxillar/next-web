...
pub type Guard<S, E> = BoxedStateGuard<S, E>;
    ...
    E: Clone + Eq,
{
    ...
    /// Gets cloned transition actions.
    pub fn cloned_actions(&self) -> Vec<Action<S, E>> {
        self.actions.clone()
    }
    ...
    pub fn guard(
        &self,
    ) -> Option<&(dyn for<'a> Fn(&'a dyn StateContext<S, E>) -> BoxFuture<'a, bool> + Send + Sync)>
    {
        self.guard.as_deref()
    }

    /// Gets a cloned guard handle if present.
    pub fn cloned_guard(&self) -> Option<Guard<S, E>> {
        self.guard.clone()
    }
