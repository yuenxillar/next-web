...
fn default_detect_xdb_file() -> String {
    let db_path = "data/ip2region.xdb";
    let prefix = "../".to_owned();
    for recurse in 1..4 {
        let filepath = prefix.repeat(recurse) + db_path;
...
pub fn searcher_init(xdb_filepath: Option<String>) {
    let xdb_filepath = xdb_filepath.unwrap_or_else(|| default_detect_xdb_file());
