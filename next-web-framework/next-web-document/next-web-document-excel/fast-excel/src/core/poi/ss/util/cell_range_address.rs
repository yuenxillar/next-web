...
impl CellRangeAddress {
    ...
    pub fn serialize(&self, out: &mut Vec<u8>) {
        out.extend_from_slice(&(self.first_row as u16).to_le_bytes());
        out.extend_from_slice(&(self.last_row as u16).to_le_bytes());
        out.extend_from_slice(&(self.first_col as u16).to_le_bytes());
        out.extend_from_slice(&(self.last_col as u16).to_le_bytes());
    }
...
impl CellReference {
    fn new(row: i32, column: i32, row_absolute: bool, column_absolute: bool) -> Self {
        CellReference {
            row,
            column,
            row_absolute,
    ...
    fn from_string(s: &str) -> Result<Self, String> {
        ...
        // Parse column letters (e.g., "A", "AB", "XFD")
        ...
        if column_end == 0 {
            return Err("No column letters found".to_string());
        }

        let column_str = &s[..column_end];
        let column = column_letters_to_index(column_str)?;

        // Parse row number
        let row_str = &s[column_end..];
        if row_str.is_empty() {
            return Err("No row number found".to_string());
        }
        ...
        Ok(CellReference::new(row - 1, column - 1, false, false))
    }

    fn format_as_string(&self) -> String {
        let column_str = index_to_column_letters(self.column + 1);
        let row_str = (self.row + 1).to_string();
...
mod tests {
    ...
    fn test_cell_range_creation() {
        let range = CellRangeAddress::new(0, 4, 0, 2);
        assert_eq!(range.get_first_row(), 0);
        assert_eq!(range.get_last_row(), 4);
        assert_eq!(range.get_first_column(), 0);
        assert_eq!(range.get_last_column(), 2);
    }

    #[test]
    #[should_panic]
    fn test_invalid_range() {
        CellRangeAddress::new(5, 3, 0, 2);
    }

    #[test]
    fn test_format_as_string() {
        let range = CellRangeAddress::new(0, 4, 0, 2);
        assert_eq!(range.format_as_string(), "A1:C5");

        let single = CellRangeAddress::new(0, 0, 0, 0);
        assert_eq!(single.format_as_string(), "A1");
    }

    #[test]
    fn test_from_string() {
        let range = CellRangeAddress::from_string("B2:D5").unwrap();
        assert_eq!(range.get_first_row(), 1); // B2 = row 1 (0-based)
        assert_eq!(range.get_last_row(), 4); // D5 = row 4 (0-based)
        assert_eq!(range.get_first_column(), 1); // B = column 1 (0-based)
        assert_eq!(range.get_last_column(), 3); // D = column 3 (0-based)

        let single = CellRangeAddress::from_string("F10").unwrap();
        assert_eq!(single.get_first_row(), 9);
        assert_eq!(single.get_last_row(), 9);
        assert_eq!(single.get_first_column(), 5);
        assert_eq!(single.get_last_column(), 5);
    }

    #[test]
    fn test_serialize_deserialize() {
        let range = CellRangeAddress::new(10, 20, 5, 15);
        let mut buffer = Vec::new();
        range.serialize(&mut buffer);

        let deserialized = CellRangeAddress::deserialize(&buffer).unwrap();
        assert_eq!(range, deserialized);
    }

    #[test]
    fn test_intersection() {
        let range1 = CellRangeAddress::new(0, 9, 0, 9);
        let range2 = CellRangeAddress::new(5, 14, 5, 14);

        let intersection = range1.intersection(&range2).unwrap();
        assert_eq!(intersection, CellRangeAddress::new(5, 9, 5, 9));

        let range3 = CellRangeAddress::new(20, 30, 20, 30);
    ...
    fn test_union() {
        ...
        assert_eq!(union, CellRangeAddress::new(0, 14, 0, 14));
    }
}
