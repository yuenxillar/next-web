...
use crate::{util::{id::unique_id, logic::Logic}, web::attrs::idempotency_attr::IdempotencyAttr};
pub(crate) fn impl_macro_idempotency(
    attr: TokenStream, 
    mut item_fn: ItemFn
) -> TokenStream {
    Logic::generate(|| {
        ...
        let extension = Ident::new(& format!("__my_extension_{}", unique_id()), Span::call_site());
        ...
        let key = key.filter(|k| !k.value().trim().is_empty()).unwrap_or(LitStr::new("Idempotency-Key", Span::call_site()));
        let name = name.filter(|k| !k.value().trim().is_empty()).unwrap_or(LitStr::new("memoryIdempotencyStore", Span::call_site()));
        let ttl =  ttl.map(|ttl| quote! { Some(#ttl) } ).unwrap_or(quote! { Some(7) });
        let cache_key =  cache_key_prefix.filter(|k| !k.value().trim().is_empty()).map(|k| {
